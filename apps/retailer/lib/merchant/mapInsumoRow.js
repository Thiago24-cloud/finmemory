...
export function mapInsumoRowToApi(row) {
  ...
  return {
    ...
    na_cesta: Boolean(row.na_cesta),
    cesta_quantidade:
      row.cesta_quantidade != null ? Number(row.cesta_quantidade) : null,
    cesta_oferta: row.cesta_oferta && typeof row.cesta_oferta === 'object' ? row.cesta_oferta : null,
    canonical_name: row.canonical_name || null,
    match_termos: Array.isArray(row.match_termos) ? row.match_termos : null,
    match_source: row.match_source || null,
    match_atualizado_em: row.match_atualizado_em || null,
