...
export function MerchantProductCard({ product, onUpdated }) {
  ...
  const img = product.image_url || product.url_imagem;
  const showStrike =
  ...
  return (
    <li className="rounded-xl border border-white/10 bg-white/[0.03] p-3 sm:p-4 flex gap-3 sm:gap-4">
      <div className="shrink-0 w-16 h-16 sm:w-20 sm:h-20 rounded-lg bg-[#0a0a10] border border-white/10 overflow-hidden">
        {img ? (
