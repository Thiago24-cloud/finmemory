...
import { Loader2, Plus, Boxes, AlertTriangle, FileText } from 'lucide-react';
...
export function MerchantInsumosSection({ onCountChange }) {
  ...
  const [stats, setStats] = useState({ total: 0, abaixo_minimo: 0 });
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      setStats({
        ...
        abaixo_minimo: data.abaixo_minimo ?? list.filter((i) => i.abaixo_minimo).length,
      });
    ...
    <section>
      ...
      <div className="rounded-2xl border border-white/10 bg-white/[0.02] p-4 sm:p-5 mb-4">
        ...
        <p className="text-[11px] text-white/45 mt-3 m-0 leading-relaxed">
          Cadastre insumos manualmente ou registre uma compra por foto/QR da nota fiscal — o estoque atualiza na hora.
        </p>
        <button
          type="button"
          onClick={() => {
          ...
          className="mt-4 w-full sm:w-auto inline-flex items-center justify-center gap-2 rounded-xl border border-[#39FF14]/45 bg-[#39FF14]/10 px-4 py-2.5 text-sm font-semibold text-[#39FF14] hover:bg-[#39FF14]/15"
        >
      ...
      {insumos.length === 0 && !formOpen && !notaFlowOpen ? (
