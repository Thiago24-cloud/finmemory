 ...
 */
import {
  fetchExternalProductImageResolved,
 ...
 * @param {{ maxUniqueNames?: number, concurrency?: number, useGoogleCse?: boolean, nameForSearch?: (p: object) => string }} opts
 */
export async function enrichMapPointsImageUrls(points, opts = {}) {
  ...
  const nameForSearch =
    typeof opts.nameForSearch === 'function' ? opts.nameForSearch : (p) => p?.product_name;

  /** Painel com image_url substitui URL antiga (ex. suplemento) antes de OFF/CSE */
  const directPairs = new Map();
  for (const p of points) {
    const searchName = String(nameForSearch(p) || p.product_name || '').trim();
    if (!searchName) continue;
    const memKey = `${normName(searchName)}\0${normName(p.store_name || '')}`;
    if (!directPairs.has(memKey)) {
      directPairs.set(memKey, { searchName, store: p.store_name || '' });
    }
  }
  const directUrlByMem = new Map();
  await Promise.all(
    [...directPairs.entries()].map(async ([memKey, v]) => {
      const f = await findDirectThumbnailRuleImageUrl(v.searchName, v.store);
      directUrlByMem.set(memKey, f && isValidResolvedImage(f) ? f : null);
    })
  );
  for (const p of points) {
    const searchName = String(nameForSearch(p) || p.product_name || '').trim();
    if (!searchName) continue;
    const memKey = `${normName(searchName)}\0${normName(p.store_name || '')}`;
    const u = directUrlByMem.get(memKey);
