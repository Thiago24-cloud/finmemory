...
import { getPrivateBetaAllowlistFromEnv, isEmailAllowedInPrivateBeta } from './privateBetaAllowlist';
 ...
 * Acesso liberado se:
 *   - sessão já tem supabaseId (login email/senha validou users no authorize), OU
 ...
 *
 * @param {string} email
 * @param {{ supabaseUserId?: string | null }} [options]
 */
export async function canAccess(email, options = {}) {
  const supabaseUserId = options?.supabaseUserId;
  if (supabaseUserId) return true;

  if (!email || typeof email !== 'string') return false;
  const normalized = email.trim().toLowerCase();
  const betaList = getPrivateBetaAllowlistFromEnv();
  if (!isEmailAllowedInPrivateBeta(normalized, betaList)) return false;
...
/** Atalho para GSSP/API: usa email + supabaseId da sessão NextAuth. */
export async function canAccessForSession(session) {
  if (!session?.user?.email) return false;
  return canAccess(session.user.email, { supabaseUserId: session.user.supabaseId });
}
