 ...
 * @param {Array<{ lat: number, lng: number, name?: string }>} stops — ordem da rota
 * @param {'driving'|'walking'|'bicycling'|'transit'} [travelMode]
 */
...
/** Abre rota multi-parada no Google Maps (máx. ~9 waypoints + destino). */
export function openGoogleMapsMultiStopRoute(origin, stops, travelMode = 'driving') {
  if (typeof window === 'undefined') return;
  const capped = (Array.isArray(stops) ? stops : []).slice(0, 10);
  const url = buildGoogleMapsMultiStopUrl(origin, capped, travelMode);
  if (!url) return;
  window.open(url, '_blank', 'noopener,noreferrer');
}
