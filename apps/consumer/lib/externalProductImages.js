 ...
 */
async function fetchGoogleCseRawDetailed(query, imageSearch, options = {}) {
  ...
  url.searchParams.set('safe', String(options.safe || 'active'));
  const requestedNum = Number.parseInt(String(options.num || (imageSearch ? 8 : 8)), 10);
  const safeNum = Math.max(1, Math.min(10, Number.isFinite(requestedNum) ? requestedNum : 8));
  url.searchParams.set('num', String(safeNum));
  if (imageSearch && options.imgType) url.searchParams.set('imgType', String(options.imgType));
  if (imageSearch && options.imgColorType) url.searchParams.set('imgColorType', String(options.imgColorType));
  if (imageSearch && options.imgDominantColor) {
    url.searchParams.set('imgDominantColor', String(options.imgDominantColor));
  }
  ...
  if (!res.ok) {
    const googleErr = googleApiErrorFromPayload(payload, res.status, res.statusText);
    const shouldRetryWhiteFallback =
      imageSearch &&
      String(options.imgColorType || '').toLowerCase() === 'white' &&
      !options._whiteFallbackRetried &&
      Number(res.status) === 400 &&
      /img_color_type|invalid value.*img_color_type|INVALID_ARGUMENT/i.test(
        String(googleErr?.message || '')
      );
    if (shouldRetryWhiteFallback) {
      return fetchGoogleCseRawDetailed(query, imageSearch, {
        ...options,
        _whiteFallbackRetried: true,
        imgColorType: 'color',
 ...
 */
async function fetchGoogleCseImageSearchDetailed(query, options = {}) {
  const q = sanitizeGoogleCseQuery(query);
  if (!q || q.length < 2) {
    return { ok: false, status: 0, items: [], googleError: { message: 'Query de busca vazia' } };
  }

  const img = await fetchGoogleCseRawDetailed(q, true, options);
  ...
  if (tryWeb) {
    const web = await fetchGoogleCseRawDetailed(q, false, options);
    if (web.ok) {
...
export async function fetchGoogleCseCuratorCandidates(productName, opts = {}) {
  ...
  async function tryQuery(q) {
    ...
    const r = await fetchGoogleCseImageSearchDetailed(trimmed, {
      // Curadoria exige foto de produto com fundo branco para cards limpos no mapa.
      imgType: 'photo',
      imgColorType: 'white',
      safe: 'active',
      num: max,
