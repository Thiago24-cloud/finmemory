...
export function normalizeQueuedProduto(raw) {
  ...
  const price = coercePositivePrice(raw?.preco ?? raw?.price ?? raw?.valor, originalName);
  const imageUrl =
...
export function splitProdutosByPublishReadiness(produtos) {
  ...
  for (const p of normalized) {
    ...
    if (!p.image_url) {
      pendingImage.push({ ...p.raw, _reason: 'missing_image', _normalized_name: p.name, preco: p.price });
      continue;
    }
    ready.push(p);
  }
