 ...
 * Contas OAuth não passam por auth_local_users — email já vem verificado pelo provedor.
 *
 * @param {import('@supabase/supabase-js').SupabaseClient} supabase
 * @param {{ email: string, name?: string, image?: string, provider?: string, providerAccountId?: string }} params
 */
export async function ensureOAuthUser(supabase, { email, name, image, provider, providerAccountId }) {
  ...
  if (existing?.id) {
    const patch = { last_sync: new Date().toISOString() };
  ...
  const { data: created, error: insertErr } = await supabase.from('users').insert(insert).select('id, email, name').single();
