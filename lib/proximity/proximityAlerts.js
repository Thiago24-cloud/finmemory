 ...
 *
 * Política Google Play: apenas localização **em primeiro plano** (FINE/COARSE via
 * `@capacitor/geolocation`). Monitoramento contínuo em segundo plano não é usado.
 */
...
function clampRadiusM(value) {
  ...
  return Math.min(PROXIMITY_RADIUS_MAX_M, Math.max(PROXIMITY_RADIUS_MIN_M, Math.round(n)));
}
...
async function loadPlugins() {
  ...
  return { Capacitor, Geolocation, LocalNotifications };
}
...
async function runProximityCheck({ LocalNotifications }, userLat, userLng, radiusM) {
...
/**
 * Para o modo de monitoramento contínuo (não há watcher em segundo plano).
 */
export async function stopProximityMonitoring({ clearTargets = true } = {}) {
  watcherId = null;
  if (clearTargets) lastTargets = [];
}

/**
 * Inicia permissões e estado para alertas; não regista watcher em background (Play policy).
 * Use `forceProximityCheckNow` enquanto o app está ao foco.
 */
export async function startProximityMonitoring({
  ...
  return { ok: true, watcherId: null, targets: lastTargets.length, foregroundOnly: true };
}
