...
export const PROXIMITY_RADIUS_PRESETS = [300, 350, 400, 500];
...
async function runProximityCheck(
  ...
  for (const row of lastTargets) {
    ...
    const body = `${row.produto_nome} em ${row.nome_loja} — ${precoLabel}. Você está a ~${Math.round(dist)} m.`;

    await LocalNotifications.schedule({
      notifications: [
        {
          id: hashToNotificationId(key + Date.now()),
          title: 'Produto da lista por perto',
          body,
