...
export async function stopProximityMonitoring({ clearTargets = true } = {}) {
  // MVP simplificado: background desativado.
  /*
  if (watcherId) {
    try {
...
export async function startProximityMonitoring({
  ...
  // MVP simplificado: monitoramento em background desativado.
  // Mantemos o fluxo antigo comentado para facilitar reativação depois.
  /*
  ...
  */
  void supabase;
  void pendingProductNames;
  void radiusInput;
  void onUnauthorized;
  void proximityConfigPartial;
  return { ok: false, reason: 'disabled_mvp' };
}
