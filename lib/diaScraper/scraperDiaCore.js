...
const { buildDiaOffersExtractionPrompt } = require('../diaOffersGptPrompt.js');
 ...
 */
/**
 * Extrai ofertas de texto HTML (import manual) via Anthropic — sem OpenAI.
 * @param {string} apiKey
 * @param {string} truncatedPlainText
 * @returns {Promise<{ store_name?: string, offers?: object[] } | null>}
 */
export async function extractOffersFromHtmlAnthropic(apiKey, truncatedPlainText) {
  const prompt = buildDiaOffersExtractionPrompt(truncatedPlainText);
  const res = await fetch('https://api.anthropic.com/v1/messages', {
    method: 'POST',
    headers: {
      'content-type': 'application/json',
  ...
  const rawText = block?.text || '';
  const jsonStr = extractJsonObject(rawText);
  if (!jsonStr) throw new Error('JSON não encontrado na resposta Anthropic');
...
/** @param {{ addressForGeocode: string, lat?: number | null, lng?: number | null }} store */
...
export function mapOfertasToProdutosFila(ofertas, sundayFallbackYmd, imageUrls = []) {
  ...
  for (let i = 0; i < list.length; i += 1) {
    ...
    const imagemUrl = imgs[i] ? String(imgs[i]).trim() : null;
    out.push({
      nome,
...
/**
 * Crons DIA (unidades do catálogo): audita na fila como aprovado e publica no mapa.
...
export async function insertApprovedFilaAndPublishScraperDia(supabase, filaRow, ownerUserId) {
  ...
  afterMapPricePointsInsert({ rows, storeName: store_name, source: SCRAPER_DIA_PRICE_SOURCE });
