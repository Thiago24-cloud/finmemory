...
/** Marca em minúsculas, sem acento — ex.: nubank, picpay, c6 */
export function resolveBankBrandLabelMinimal(params = {}) {
  const theme = getBankTheme({
    bankIdentity: params.name,
    connectorName: params.connectorName,
    connectorId: params.connectorId,
  ...
  if (theme.key && theme.key !== 'default') return theme.key;

  const label = resolveBankBrandLabel(params);
  return label
    .toLowerCase()
    .normalize('NFD')
    .replace(/[\u0300-\u036f]/g, '')
...
/** Simulador / listas compactas — ex.: "nubank credito", "nubank debito" */
export function formatBankAccountDisplayNameMinimal(params = {}) {
  const brand = resolveBankBrandLabelMinimal(params);
  const kind = inferAccountKind(params.accountType, params.name);
  const suffix = kind === ACCOUNT_KIND_CREDITO ? 'credito' : 'debito';
  return `${brand} ${suffix}`;
}
