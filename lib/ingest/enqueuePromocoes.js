 ...
 */
import { triggerImageEnrichmentAsync } from '../catalog/triggerImageEnrichment.js';
 ...
 *   origem: string,
 * }} payload
 * @returns {Promise<{ ok: boolean, filaId?: string, error?: string }>}
 */
export async function enqueuePromocoes(supabase, payload) {
  ...
  const needsImage = produtos.some(
    (p) => !String(p?.imagem_url || p?.image_url || '').trim() && p?.tentativa_busca !== true
  );
  if (data?.id && needsImage) {
    triggerImageEnrichmentAsync({ filaId: data.id, mode: 'bot_fila', limit: 60 });
  }

  return { ok: true, filaId: data.id };
}
