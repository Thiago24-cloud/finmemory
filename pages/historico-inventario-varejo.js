...
export async function getServerSideProps(ctx) {
  try {
    ...
    if (!isVarejistaUser(session.user)) {
      return { redirect: { destination: '/scan-product', permanent: false } };
    }
    return { props: {} };
  } catch {
    return { redirect: { destination: '/login?callbackUrl=/historico-inventario-varejo', permanent: false } };
...
export default function HistoricoInventarioVarejoPage() {
  ...
  const handleShareLote = async (lote) => {
    ...
    try {
      ...
      const mRes = await completeDailyMission('varejo_exportar_csv');
      if (mRes.ok) {
        const mJson = await mRes.json();
        if (mJson.xp_awarded > 0) {
          toast.success(`+${mJson.xp_awarded} XP — CSV compartilhado!`, {
            className: 'border border-primary/40 shadow-[0_0_24px_rgba(46,204,73,0.25)]',
          });
        } else if (outcome === 'fallback') {
          toast.message('Arquivo baixado e copiado — envie pelo WhatsApp Web ou e-mail.');
        } else {
