-- find_or_create_store: menos duplicatas por geocode/nome.
-- 1) CNPJ 14 dígitos: também encontra loja inativa (mesmo CNPJ), reativa active=true.
-- 2) Camadas nome/endereço: raio 200 m -> 400 m (geocode costuma errar 100–300 m).
...
BEGIN
  ...
  IF length(v_cnpj_digits) = 14 THEN
    ...
    -- Camada 0b: mesmo CNPJ em loja inativa (ex.: removida do mapa) — reutiliza pin, reativa
    SELECT s.id INTO v_found
    FROM public.stores s
...
COMMENT ON FUNCTION public.find_or_create_store(text, text, double precision, double precision, text) IS
  'Deduplica: (0a) CNPJ ativo (0b) CNPJ inativo reativa (1) nome+400m sim>0,6 (2) endereço+400m sim>0,45 (3) insert.';
